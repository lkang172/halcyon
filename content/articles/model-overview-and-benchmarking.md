...
However, we will begin by providing a concise summary of the model landscape:
...
Benchmarks measure some aspect of a model. Common benchmarks include price per output token, coding and software engineering, end-to-end latency, context retention, reasoning, and human preference.

The main groups that create benchmarks are aggregators, academics, nonprofits and independent research organizations, and frontier labs themselves.

### Aggregators and leaderboards

These run existing tests under a standardized setup, and publish rankings. This directly compares model performances with each other under identical conditions.

Artificial Analysis has an intelligence score, a weighted index of ten separate evaluations. BenchLM similarly aggregates benchmark results from various sources and compares them.
